...
if (isset($_POST['update'])) {
  $req_fields = array('name', 'username', 'level', 'department');
  validate_fields($req_fields);
  if (empty($errors)) {
    $id = (int) $e_user['id'];
    $name = remove_junk($db->escape($_POST['name']));
    $username = remove_junk($db->escape($_POST['username']));
    $dep = remove_junk($db->escape($_POST['department']));
    $level = (int) $db->escape($_POST['level']);
    $status = remove_junk($db->escape($_POST['status']));
    $sql = "UPDATE users SET name ='{$name}', username ='{$username}',user_level='{$level}',status='{$status}',department='{$dep}' WHERE id='{$db->escape($id)}'";
    $result = $db->query($sql);
    ...
    <div class="panel panel-default">
      <div class="panel-heading">
        <strong>
          ...
          Update
          <?php echo remove_junk(ucwords($e_user['name'])); ?> Account
        </strong>
      </div>
          ...
          <div class="form-group">
            ...
            <select class="form-control" name="department">
              <?php foreach ($deps as $dep): ?>
                <option <?php if ($dep['dep_name'] === $e_user['department'])
                  echo 'selected="selected"'; ?>
                  value="<?php echo $dep['dep_name']; ?>">
                  <?php echo ucwords($dep['dep_name']); ?>
                </option>
              <?php endforeach; ?>
            </select>
          </div>
    ...
    <div class="panel panel-default">
      <div class="panel-heading">
        <strong>
          <span class="glyphicon glyphicon-th"></span>
          Change
          <?php echo remove_junk(ucwords($e_user['name'])); ?> password
        </strong>
      </div>
      <div class="panel-body">
