...
if (isset($_GET['POL_id'])) {
    $qry = $db->query("SELECT p.*,s.COMPANY_NAME as supplier FROM purchase_order_list p inner join
    supplier s on p.supplier_id = s.SUPPLIER_ID where p.POL_id = '{$_GET['POL_id']}'");
    if ($qry->num_rows > 0) {
            ...
            <div class="container-fluid">
                    ...
                    <div class="col-md-6">
                        <div class="form-group">
                            ...
                            <select name="supplier_id" id="supplier_id" class="custom-select select2">
                                ...
                                <?php
                                $supplier = $db->query("SELECT * FROM `supplier` where status = 1 order by `COMPANY_NAME` asc");
                                while ($row = $supplier->fetch_assoc()):
                                    ?>
                                    <option value="<?php echo $row['SUPPLIER_ID'] ?>" <?php echo isset($supplier_id) && $supplier_id == $row['SUPPLIER_ID'] ? "selected" : "" ?>>
                ...
                <fieldset>
                    ...
                    <div class="row justify-content-center align-items-end">
                        ...
                        $item_arr = array();
                        $cost_arr = array();
                        $item = $db->query("SELECT * FROM `products` order by `prod_name` asc");
                        while ($row = $item->fetch_assoc()):
                            $item_arr[$row['supplier_id']][$row['prod_id']] = $row;
                            $cost_arr[$row['prod_id']] = $row['buy_price'];
                        endwhile;
                        ?>
                        <div class="col-md-3">
                            <div class="form-group">
                                ...
                                <select id="item_id" class="custom-select ">
                                    <?php foreach ($prods as $prod): ?>
                                        <option value="<?php echo $prod['prod_id']; ?>">
                                            <?php echo ucwords($prod['prod_name']); ?>
                                        </option>
                                    <?php endforeach; ?>
                                </select>
                            </div>
                ...
                <table class="table table-striped table-bordered" id="list">
                    ...
                    <tbody>
                        ...
                        $total = 0;
                        if (isset($id)):
                            $qry = $db->query("SELECT p.*,i.prod_name FROM `po_items` p inner join products i on p.item_id = i.prod_id where p.po_id = '{$id}'");
...
$(function () {
    ...
    $('#po-form').submit(function (e) {
        ...
        start_loader();
        $.ajax({
            url: "../includes/sql.php?f=save_po",
            ...
            success: function (resp) {
                if (resp.status == 'success') {
                    location.replace("view_po&id=" + resp.id);
    ...
    function rem(_this) {
        ...
        calc()
        if ($('table#list tbody tr').length <= 0) $('#supplier_id').removeAttr('readonly')
    } function calc() {
        var
            sub_total = 0; var grand_total = 0; var discount = 0; var tax = 0; $('table#list tbody input[name="total[]"
            ]').each(function () { sub_total +=parseFloat($(this).val()) }) $('table#list tfoot
                .sub - total').text(parseFloat(sub_total).toLocaleString('en - US', { style: 'decimal' , maximumFractionDigit: 2
            })) var discount = sub_total * (parseFloat($('[name=" discount_perc" ]').val()) / 100) sub_total = sub_total -
        discount; var tax = sub_total * (parseFloat($('[name="tax_perc" ]').val()) / 100) grand_total = sub_total + tax
    $('.discount').text(parseFloat(discount).toLocaleString('en-US', {
        style: 'decimal', maximumFractionDigit:
            2
    })) $('[name="discount" ]').val(parseFloat(discount))
    $('.tax').text(parseFloat(tax).toLocaleString('en-US', { style: 'decimal', maximumFractionDigit: 2 }))
    $('[name="tax" ]').val(parseFloat(tax)) $('table#list tfoot
        .grand - total').text(parseFloat(grand_total).toLocaleString('en - US', { style: 'decimal' ,
            maximumFractionDigit: 2 })) $('[name="amount" ]').val(parseFloat(grand_total)) } </script>
