...
<div class="card card-outline card-primary">
    <div class="card-header">
        ...
        <div class=" card-tools">
            <a href="manage_po" class="btn btn-flat btn-primary"><span class="fas fa-plus"></span> Create New</a>
        </div>
    </div>
    <div class="card-body">
            ...
            <div class="container-fluid">
                <table class="table table-bordered table-stripped">
                    ...
                    <tbody>
                        <?php
                        $i = 1;
                        $qry = $db->query("SELECT p.*, s.COMPANY_NAME as supplier FROM `purchase_order_list` p inner join supplier s on p.supplier_id = s.SUPPLIER_ID order by p.`date_created` desc");
                        while ($row = $qry->fetch_assoc()):
                            $row['items'] = $db->query("SELECT count(item_id) as `items` FROM `po_items` where po_id = '{$row['POL_id']}' ")->fetch_assoc()['items'];
                            ?>
                            <tr>
                                    ...
                                    <div class="dropdown-menu" role="menu">
                                        <?php if ($row['status'] == 0): ?>
                                            ...
                                            <a class="dropdown-item"
                                                ...
                                                data-id="
                                <?php echo $row['id'] ?>"><span class="fa fa-boxes text-dark"></span>
                                                Receive
                                            </a>
                                        ...
                                        <a class="dropdown-item"
                                            ...
                                            data-id="
                            <?php echo $row['id'] ?>"><span class="fa fa-eye text-dark"></span>
                                            View</a>
                                        <div class="dropdown-divider"></div> <a class="dropdown-item"
                                            href=" <?php echo base_url . 'admin?page=purchase_order/manage_po&id=' . $row['id'] ?>"
                                            data-id="
                                <?php echo $row['id'] ?>"><span class="fa fa-edit text-primary"></span>
                                            Edit</a>
                                        <div class="dropdown-divider"></div>
